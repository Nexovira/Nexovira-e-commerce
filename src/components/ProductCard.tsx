      ...
      <div className="p-2.5 sm:p-4 flex flex-col flex-1 justify-between gap-2 sm:gap-3">
        <div>
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 mb-1">
            <span className="font-extrabold text-cyan-600 uppercase tracking-wider truncate">
              {product.brand}
            </span>
            <span className="truncate hidden sm:inline">{product.categoryName}</span>
          </div>

          {/* Product Title */}
          <h3
            onClick={() => onViewDetail(product)}
            className="text-xs sm:text-sm font-bold text-slate-900 hover:text-cyan-600 cursor-pointer line-clamp-2 leading-snug transition-colors"
          >
            {product.name}
          </h3>
          ...
          <div className="flex items-center gap-1 mt-1 sm:mt-2">
            <div className="flex items-center text-amber-500">
              ...
              <span className="text-[11px] font-bold ml-1 text-slate-800">{product.rating}</span>
            </div>
            <span className="text-[10px] text-slate-400 hidden xs:inline">({product.reviewCount})</span>
          </div>
        ...
        {/* Price & Action Row */}
        <div className="pt-2 sm:pt-3 border-t border-slate-100 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2">
          <div>
          ...
          <button
            ...
            className={`w-full xs:w-auto px-3 py-2 min-h-[44px] rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-xs ${
              isOutOfStock
